...
def genDistInvPer(Rin, Ncells, Np, L, av = [0.0, 0.0], std = [1.0, 1.0]):
    ...
    # the input has dimensions (Nsamples, Ncells*Np)
    # the ouput has dimensions (Nsamples*Ncells*Np*(3*Np-1), 2)
    # choose five points in the neighbor 
    # add assert with respect to the input shape 
    Nsamples = Rin.shape[0]
    R = tf.reshape(Rin, (Nsamples, Ncells, Np))
    ...
    for i in range(Ncells):
        for j in range(Np):
            ...
            for l in range(-1,2):
                for k in range(Np):
                    if j != k or l != 0: ##local interaction?
                        a = tf.subtract(R[:,(i+l)%Ncells,k], R[:,i,j])
                        b = a - L*tf.round(a/L) ##avoid out of border
...
def genDistInvPeradjusted(Rin, Ncells, Np, L,leftindex,rightindex,av = [0.0, 0.0], std = [1.0, 1.0]):
    # function to generate the generalized coordinates for periodic data
    # the input has dimensions (Nsamples, Ncells*Np)
    # the ouput has dimensions (Nsamples*Ncells*Np*(3*Np-1), 2)
    # choose five points in the neighbor 
    ...
    Nsamples = Rin.shape[0]
    R = tf.reshape(Rin, (Nsamples, Ncells, Np))
    ###a,b are required to be checked carefully
    Abs_Array = []
    Abs_Inv_Array = []
    # we compute the difference between points in the same
    # interaction list
    for i in range(Ncells):
        for j in range(Np):
            absDistArray = []
            absInvArray = []
            for l in range(leftindex,rightindex):
                for k in range(Np):
                    if j != k or l != 0: ##local interaction?
                        a = tf.subtract(R[:,(i+l)%Ncells,k], R[:,i,j])
                        b = a - L*tf.round(a/L) ##avoid out of border
    ...
    # asserting the final size of the tensor
    assert R_Diff_total.shape[0] == Nsamples*Ncells*Np*((rightindex-leftindex)*Np-1)
...
class MyDenseLayerNoBias(tf.keras.layers.Layer):
  def __init__(self, num_outputs):
    super(MyDenseLayerNoBias, self).__init__()
    self.num_outputs = num_outputs
...
def train_stepv2(model,Ncells,Np,L,av,std,avNUFFT,stdNUFFT, optimizer, loss, 
               inputs, outputsE,
               weightE):
# funtion to perform one training step
  with tf.GradientTape() as tape:
    # we use the model the predict the outcome
    predE = model(inputs,Ncells,Np,L,av,std,avNUFFT,stdNUFFT, training=True)
...
def train_step_two(model, optimizer, loss, 
               inputs1, inputs2, outputs1E, outputs2E,
               weightE):
# funtion to perform one training step
  with tf.GradientTape() as tape:
    # we use the model the predict the outcome
    pred1E = model(inputs1, training=True)
    pred2E = model(inputs2, training=True)
    # fidelity loss usin mse
    total_loss = weightE*loss(pred1E, outputs1E) + weightE*loss(pred2E, outputs2E)
  # compute the gradients of the total loss with respect to the trainable variables
  gradients = tape.gradient(total_loss, model.trainable_variables)
  # update the parameters of the network
  optimizer.apply_gradients(zip(gradients, model.trainable_variables))
...
#def train_step_shortrange(model, optimizer, loss, 
#               inputs, outputsE,
#               weightE):
## funtion to perform one training step
#  with tf.GradientTape() as tape:
...
def computeLossIndex(model, optimizer, loss,
...
def computeLoss(model, optimizer, loss,
                    inputs, outputsF, 
                    weightF):
# funtion to perform one training step when predicting both the
# potential and the forces
#  with tf.GradientTape() as tape:
    # we use the model the predict the outcome
    predF = model(inputs, training=True)
...
class FFTLayer(tf.keras.layers.Layer):
  ...
  def call(self, input):
    ...
    #in frequency domain, channel multiply fft.
    #in real domain, do convolution
...
class NUFFTLayer(tf.keras.layers.Layer):###NUFFT 
  # this layers uses a few kernels to approximate exp(-mu)
  # and we add the exact mu to check if that becomes worse
  ...
  def call(self, input):
    ...
    fftGauss = tf.signal.fftshift(
               tf.signal.fft(arrayReducGaussian))/\
                             (2*np.pi*self.NpointsMesh/self.L) ##width in the domain
...
class NUFFTLayerMultiChannelInitMixedImzero(tf.keras.layers.Layer):
  # this layers uses a few kernels to approximate exp(-mu)
  # and we add the exact mu to check if that becomes worse
  def __init__(self, nChannels, NpointsMesh, tau, xLims, mu1 = 1.0, mu2=0.5):
    super(NUFFTLayerMultiChannelInitMixedImzero, self).__init__()
    self.nChannels = nChannels
    self.NpointsMesh = NpointsMesh 
    self.mu1 = tf.constant(mu1, dtype=tf.float32)
    self.mu2 = tf.constant(mu2, dtype=tf.float32)
  ...
  def build(self, input_shape):
    ...
    self.multipliersIm.append( self.add_weight("multIm_0",
                       initializer=tf.initializers.zeros(), 
                       shape = (1, self.NpointsMesh),trainable=False) )
...
#  @tf.function
  def call(self, input):
    ...
    # we add them together
    arrayReducGaussian = tf.complex(tf.reduce_sum(array_normal, axis = 1), 0.0)
    # (batch_size, NpointsMesh) (we sum the gaussians together)neighbor
    # we apply the fft
    print("computing the FFT")
...
def genDistInvPerNlist(Rin, Npoints, neighList, L, av = [0.0, 0.0], std = [1.0, 1.0]):
    ...
    # interact points are defined by neighList
    # add assert with respect to the input shape 
    Nsamples = Rin.shape[0]
    ...
    Idx1 = tf.constant(np.linspace(0,Nsamples-1, Nsamples).astype(np.int32).reshape((-1,1)))
...
def genDistInvPerNlistIndex(Rin, neighList, Np, Lcell , av = [0.0, 0.0], std = [1.0, 1.0]):
    # function to generate the generalized coordinates for periodic data
    # the input has dimensions (Nsamples, Ncells*Np)
    # the ouput has dimensions (Nsamples*Ncells*Np*(3*Np-1), 2)
...
def computInterList( Rinnumpy, L,  radious, maxNumNeighs):
  ...
  # computing the corresponding distance
...
class NUFFTLayerMultiChannelInitMixedEven(tf.keras.layers.Layer):
  # this layers uses a few kernels to approximate exp(-mu)
  # and we add the exact mu to check if that becomes worse
  def __init__(self, nChannels, NpointsMesh, tau, xLims, mu1 = 1.0, mu2=0.5):
    super(NUFFTLayerMultiChannelInitMixedEven, self).__init__()
    self.nChannels = nChannels
    self.NpointsMesh = NpointsMesh 
    self.mu1 = tf.constant(mu1, dtype=tf.float32)
    self.mu2 = tf.constant(mu2, dtype=tf.float32)
  ...
  def build(self, input_shape):
    ...
    xExp = tf.expand_dims(4*np.pi*tf.math.reciprocal(tf.square(self.kGrid[:self.NpointsMesh//2+1]) + \
                                  tf.square(self.mu1)), 0)

    initKExp = tf.keras.initializers.Constant(xExp.numpy())

    xExp2 = tf.expand_dims(4*np.pi*tf.math.reciprocal(tf.square(self.kGrid[:self.NpointsMesh//2+1]) + \
                                  tf.square(self.mu2)), 0)
    ...
    ##### initialize the real multiplier
    self.multipliersRe.append(self.add_weight("multRe_0",initializer=initKExp, 
                                              shape = (1, self.NpointsMesh//2+1)))
    self.multipliersRe.append(xExp[0,self.NpointsMesh//2-1::-1][None,:])       
    self.multipliersRe.append(self.add_weight("multRe_1",initializer=initKExp2, 
                                              shape = (1, self.NpointsMesh//2+1)))
    self.multipliersRe.append(xExp2[0,self.NpointsMesh//2-1::-1][None,:])
...
#  @tf.function
  def call(self, input):
    ...
    print("applying the multipliers")
    #### do a symmetric extension
    self.multipliersRe[1] = self.multipliersRe[0][0,self.NpointsMesh//2-1::-1][None,:]
    mult_temp = tf.concat([self.multipliersRe[0],self.multipliersRe[1]],axis=1)

    multReRefft = tf.multiply(mult_temp,Rerfft)
    multReImfft = tf.multiply(self.multipliersIm[0],Rerfft)
    multImImfft = tf.multiply(mult_temp,Imrfft)
    multImRefft = tf.multiply(self.multipliersIm[0],Imrfft)
    ...
    #### do a symmetric extension
    self.multipliersRe[3] = self.multipliersRe[2][0,self.NpointsMesh//2-1::-1][None,:]
    mult_temp1 = tf.concat([self.multipliersRe[2],self.multipliersRe[3]],axis=1)
    # multfft = tf.multiply(self.multChannels*rfft)
    multReRefft2 = tf.multiply(mult_temp1,Rerfft)
    multReImfft2 = tf.multiply(self.multipliersIm[1],Rerfft)
    multImImfft2 = tf.multiply(mult_temp1,Imrfft)
    multImRefft2 = tf.multiply(self.multipliersIm[1],Imrfft)
...
class NUFFTLayerInitMixedEven(tf.keras.layers.Layer):
  # this layers uses a few kernels to approximate exp(-mu)
  # and we add the exact mu to check if that becomes worse
  def __init__(self, nChannels, NpointsMesh, tau, xLims, mu1 = 1.0, mu2 = 0.5):
    super(NUFFTLayerInitMixedEven, self).__init__()
    self.nChannels = nChannels
    self.NpointsMesh = NpointsMesh 
    self.mu1 = tf.constant(mu1, dtype=tf.float32)
    self.mu2 = tf.constant(mu2, dtype=tf.float32)
  ...
  def build(self, input_shape):
    ...
    #self.multipliersRe.append(tf.concat([Re0, Re0[0,-2::-1][None,:]],axis=1))
    self.multipliersRe.append(self.add_weight("multRe_0",initializer=initKExp, 
                                              shape = (1, self.NpointsMesh//2+1)))
    self.multipliersRe.append(xExp[0,self.NpointsMesh//2-1::-1][None,:])
#    self.multipliersRe[0] = tf.concat([self.multipliersRe[0], self.multipliersRe[0][0,-2::-1][None,:]],axis=1)       
    self.multipliersRe.append(self.add_weight("multRe_1",initializer=initKExp2, 
                                              shape = (1, self.NpointsMesh//2+1),
                                              trainable=False) )
    self.multipliersRe.append(xExp2[0,self.NpointsMesh//2-1::-1][None,:])
...
#  @tf.function
  def call(self, input):
    ...
    print("applying the multipliers")
    #print(self.multipliersRe[0][0,490:510])
#    tf.compat.v1.assign(self.multipliersRe[0][0,self.NpointsMesh//2+1:],
#                        self.multipliersRe[0][0,self.NpointsMesh//2-1::-1])
    # multfft = tf.multiply(self.multChannels*rfft)
    self.multipliersRe[1] = self.multipliersRe[0][0,self.NpointsMesh//2-1::-1][None,:]
    mult_temp = tf.concat([self.multipliersRe[0],self.multipliersRe[1]],axis=1)
#    self.multipliersRe[0] = tf.concat([self.multipliersRe[0][0,:self.NpointsMesh//2+1][None,:],
#                       self.multipliersRe[0][0,self.NpointsMesh//2-1::-1][None,:]],axis=1)
    #self.multipliersRe[0][0,self.NpointsMesh//2+1:] = self.multipliersRe[0][0,self.NpointsMesh//2-1::-1]
    multReRefft = tf.multiply(mult_temp,Rerfft)
    multReImfft = tf.multiply(self.multipliersIm[0],Rerfft)
    multImImfft = tf.multiply(mult_temp,Imrfft)
    multImRefft = tf.multiply(self.multipliersIm[0],Imrfft)

...
#                                                  +  tf.square(self.mu0)), 0)
    ...
    #print('multfftDeconv1',multfftDeconv1.shape)
    ...
    #print('irfft1',irfft1.shape)
    diag_sum1 = tf.reduce_sum(tf.reduce_sum(irfft1*array_gaussian,axis=-1),axis=-1)
    #print('diag_sum1',diag_sum1.shape)
    ##(Nsamples,Np*Ncells) part energy
    local1 = tf.reduce_sum(tf.reduce_sum(tf.reduce_sum(irfft1,axis=1,keepdims=True)*array_gaussian,axis=-1),axis=-1)    
    ##(Nsamples,Np*Ncells)
    #print('local1',local1.shape)
    energy1 = local1 - diag_sum1
    #print('energy1',energy1.shape)
    ...
    ##(Nsamples,Np*Ncells) part energy
    local2 = tf.reduce_sum(tf.reduce_sum(tf.reduce_sum(irfft2,axis=1,keepdims=True)*array_gaussian,axis=-1),axis=-1)

# -*- coding: utf-8 -*-
"""
Created on Tue May  5 21:24:02 2020
...
from utilities import genDistInvPerNlist, computInterList,trainStepList
from utilities import MyDenseLayer, pyramidLayer
from utilities import NUFFTLayerMultiChannelInitOneSided
...
fftChannels = 2
xLims = [0.0, 10.0]
sigmaFFT = 10*(xLims[1]-xLims[0])/NpointsFourier
...
# dimension are (Nsamples, Npoints and MaxNumneighs) right
neighList = tf.Variable(Idx)
Npoints = Np*Ncells

#Rradious = L

#LongRangeIdx = computInterList(Rinnumpy, L,  Rradious, maxNumNeighs)
## dimension are (Nsamples, Npoints and MaxNumneighs) right
#LongRangeneighList = tf.Variable(LongRangeIdx)
...
##############################
NUFFTinit = NUFFTLayerMultiChannelInitOneSided(fftChannels, \
...
IdxTest = computInterList(pointsTest, L,  radious, maxNumNeighs)
# dimension are (Nsamples, Npoints and MaxNumneighs) right
...
class DeepMDsimpleEnergy(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.layerPyramidInv  = pyramidLayer(descripDim, 
                                       actfn = tf.nn.tanh)
    self.NUFFTLayerMultiChannelInitOneSided = NUFFTLayerMultiChannelInitOneSided(fftChannels, \
      NpointsFourier, sigmaFFT, xLims, mu)
    self.layerPyramidLongRange  = pyramidLayer(descripDim, 
                                       actfn = tf.nn.relu)####relu here
...
#  @tf.function
  def call(self, inputs, neighList):
    with tf.GradientTape() as tape:
      ...
      longRangewCoord = (self.NUFFTLayerMultiChannelInitOneSided(inputs) - \
                         self.avNUFFT)/ self.stdNUFFT
      ...
      Dtemp = tf.reshape(LL, (-1, maxNumNeighs, 

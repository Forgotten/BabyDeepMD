...
from utilities import genDistInvPerLongRange
from utilities import genDistPerLongRangeFull
from utilities import MyDenseLayer, pyramidLayer
from utilities import pyramidLayerNoBias, fmmLayerExpTrainablePer
import os
...
ExtCoords = genDistPerLongRangeFull(Rinput[0:1000,:], Ncells, 
                                    Np, L,
                                    [0.0, 0.0], 
...
class DeepMDsimpleEnergy(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.fmmLayerExpTrainablePer = fmmLayerExpTrainablePer(Ncells, Np, L)
  ...
  def call(self, inputs):
    with tf.GradientTape() as tape:
      ...
      genCoordinates = genDistInvPer(inputs, self.Ncells, 
                                     self.Np, self.L,
                                     self.av, self.std)
      ...
      longRangewCoord = self.fmmLayerExpTrainablePer(inputs)
...
## Defining the model
model = DeepMDsimpleEnergy(Np, Ncells, L,
                           filterNet, fittingNet, mu,
                           av, std, 
                           meanLongRange, stdLongRange)
...
print( "values of the trainable decay %f" %(model.fmmLayerExpTrainablePer.mu.numpy()))
...
#   # (Nsamples, Ncells*Np)
#   # in this case we are only considering the distances
#   genCoordinates = genDistInvPer(RinputSmall, model.Ncells, model.Np, 
#                               model.av, model.std)
...
#   # we compute the FMM and the normalize by the number of particules
#   longRangewCoord = model.fmmLayerExpTrainablePer(RinputSmall)
#   # (Nsamples, Ncells*Np, 4) # we are only using 4 kernels

...
from utilities import pyramidLayerNoBias, NUFFTLayerMultiChannelInitOneSided
...
class DeepMDsimpleForces(tf.keras.Model):
  ...
  def call(self, inputs):
    ...
    with tf.GradientTape() as tape:
      ...
      # longRangewCoord = (self.NUFFTLayerMultiChannelInitOneSided(inputs) - np.reshape(np.array([404.24948, 104.75703]), (1,1,2)))/\
...
# print( "values of the trainable decay %f" %(model.NUFFTLayerMultiChannelInitOneSided.mu.numpy()))
...
#   # (Nsamples*Ncells*Np*(3*Np - 1), descriptorDim)
#   # we compute the FMM and the normalize by the number of particules
#   longRangewCoord = model.NUFFTLayerMultiChannelInitOneSided(Rinput)
#   # (Nsamples, Ncells*Np, 1) # we are only using 4 kernels
#   # we normalize the output of the fmm layer before feeding them to network

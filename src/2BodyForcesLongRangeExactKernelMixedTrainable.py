...
from utilities import pyramidLayerNoBias, fmmLayerExpTrainable
...
class DeepMDsimpleEnergy(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.fmmLayerExpTrainable = fmmLayerExpTrainable(Ncells, Np)
  ...
  def call(self, inputs):
    with tf.GradientTape() as tape:
      ...
      longRangewCoord = self.fmmLayerExpTrainable(inputs)
...
#   # we compute the FMM and the normalize by the number of particules
#   longRangewCoord = model.fmmLayerExpTrainable(RinputSmall)
#   # (Nsamples, Ncells*Np, 4) # we are only using 4 kernels

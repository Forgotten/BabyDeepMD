...
from utilities import pyramidLayerNoBias, NUFFTLayerMultiChannel
...
class DeepMDsimpleForces(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.NUFFTLayerMultiChannel = NUFFTLayerMultiChannel(fftChannels, NpointsFourier, sigmaFFT, xLims)
...
# print( "values of the trainable decay %f" %(model.NUFFTLayerMultiChannel.mu.numpy()))
...
#   # (Nsamples*Ncells*Np*(3*Np - 1), descriptorDim)
#   # we compute the FMM and the normalize by the number of particules
#   longRangewCoord = model.NUFFTLayerMultiChannel(Rinput)
#   # (Nsamples, Ncells*Np, 1) # we are only using 4 kernels
#   # we normalize the output of the fmm layer before feeding them to network

...
from utilities import genDistInvPerNlistVec, trainStepList, computInterList
from utilities import MyDenseLayer, pyramidLayer
from utilities import pyramidLayerNoBias

from utilities import fmmLayerExpTrainablePer

# change to periodic version 
...
ExtCoords = genDistPerLongRangeFull(Rinput[0:1000,:], Ncells, Np, L, 
...
class DeepMDsimpleEnergy(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.fmmLayerExpTrainablePer = fmmLayerExpTrainablePer(self.Ncells, 
                                                           self.Np, 
                                                           self.L)
...
## Defining the model
model = DeepMDsimpleEnergy(Npoints, Np, Ncells, maxNumNeighs,
...
neighList_test = tf.Variable(Idx)
...
# we need to modify this one later
  ...
  for epoch in range(epochs):
    ...
    potPred, forcePred = model(pointsTest, neighList_test)

...
class NUFFTLayerEnergy1DParametrized(tf.keras.layers.Layer):
  # parametrized version of the NUFFT layer 
  # and we add the exact mu to check if that becomes worse
  def __init__(self, nChannels, NpointsMesh, 
               xLims, mu1 = 1.0, mu2=0.5):
    super(NUFFTLayerEnergy1DParametrized, self).__init__()
  ...
  def call(self, input):
    ...
    # (batch_size, Np*Ncells, NpointsMesh)
    Deconv = tf.complex(tf.expand_dims(tf.expand_dims(gaussianDeconv1D(self.kGrid, self.tau), 0),0),0.0)
    #(1, 1, NpointsMesh)

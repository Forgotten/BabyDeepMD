...
from utilities import genDistInvPerNlist2DRadialArray, trainStepEnergyList, computInterList2DOpt
...
# TODO: this should be modify for the forces
...
# # we only consider the first 8 snapshots 
Rin = Rinput[:8,:,:]
...
# genCoordinates = genDistInvPerNlist2DRadialArray(Rin, Npoints, 
#                                             neighList,radious, 
#                                             smoothRadious,  L)
...
class DeepMDsimpleEnergy(tf.keras.Model):
  ...
  def call(self, inputs, neighList):
    ...
    # in this case we are only considering the distances
    genCoordinates = genDistInvPerNlist2DRadialArray(inputs, self.Npoints, 
                                                neighList, self.L, 
                                                self.av, self.std)  # this need to be fixed
    # (Nsamples*Npoints*maxNumNeighs, 1)
    # the L1 and L2 functions only depends on the first entry
    G1   = self.layerPyramid(genCoordinates)
    # (Nsamples*Npoints*maxNumNeighs, descriptorDim)
            
    # here we need to assemble the Baby Deep MD descriptor
    genCoord = tf.reshape(genCoordinates, (-1, self.maxNumNeighs, 1))
    # (Nsamples*Npoints, maxNumNeighs, 1)
    ...
    # (Nsamples*Npoints, 1, descriptorDim)
    Omega2 =  tf.matmul(genCoord, G2_reshape, transpose_a = True)
    # (Nsamples*Npoints, 1, descriptorDim//2)
    ...
    return Energy # we only compute the energy

avTF = tf.constant([0.0, 0.0], dtype=tf.float32)
stdTF = tf.constant([1.0, 1.0], dtype=tf.float32)
Npointstf = tf.constant(Npoints, dtype = tf.int32)

print("Building the model")
## Defining the model
model = DeepMDsimpleEnergy(Npoints, Ltf, radious, smoothRadious, 
                           maxNumNeighs, filterNet, fittingNet, 
                           avTF, stdTF)

# quick run of the model to check that it is correct.
# we use a small set 
E = model(Rin, neighList)
...
# we need to modify this one later
  ...
  for epoch in range(epochs):
    ...
    for step, x_batch_train in enumerate(train_dataset):
      ...
      loss = trainStepEnergyList(model, optimizer, mse_loss_fn,
                                 x_batch_train[0], neighList,
                                 x_batch_train[1], weightE)
      loss_metric(loss)
...
potentialTestRscl = potentialTest/forcesStd

potPred = model(pointsTest, neighList)

err = tf.sqrt(tf.reduce_sum(tf.square(potPred - potentialTestRscl)))/tf.sqrt(tf.reduce_sum(tf.square(potentialTestRscl)))
...
#     # in this case we are only considering the distances
# genCoordinates = genDistInvPerNlist2DRadialArray(inputs, model.Npoints, 
#                                             neighList, model.L, 
#                                             model.av, model.std) # this need to be fixed

# # (Nsamples*Npoints*maxNumNeighs, 1)
# # the L1 and L2 functions only depends on the first entry
# G1   = model.layerPyramid(genCoordinates)
# # (Nsamples*Npoints*maxNumNeighs, descriptorDim)
         
# # here we need to assemble the Baby Deep MD descriptor
# genCoord = tf.reshape(genCoordinates, (-1, model.maxNumNeighs, 1))
# # (Nsamples*Npoints, maxNumNeighs, 1)

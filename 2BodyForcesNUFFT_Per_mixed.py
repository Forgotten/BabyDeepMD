...
checkFolder  = "/"
checkFile = checkFolder + "checkpoint" + \
...
NUFFTinit = NUFFTLayerMultiChannelInit(fftChannels, \
      NpointsFourier, sigmaFFT, xLims, mu1)
...
#  @tf.function
  def call(self, inputs):
    ...
    with tf.GradientTape() as tape:
      ...
      longRangewCoord = (self.NUFFTLayerMultiChannelInit(inputs) - self.avNUFFT)/ self.stdNUFFT
      ...
      # (Nsamples*Ncells*Np, ((left-right)*Np - 1), 2*descriptorDim)
...
model = DeepMDsimpleForces(Np, Ncells, 
                           filterNet, fittingNet, mu1, leftindex, rightindex, 
...
lrSchedule = tf.keras.optimizers.schedules.ExponentialDecay(
             initialLearningRate,
             decay_steps= 2000, ##here I make an adjustment
